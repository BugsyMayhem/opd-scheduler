...
if col_undo.button(f"↩️ Undo Action ({h_count})", use_container_width=True, disabled=(h_count == 0)):
    st.session_state.main_df = st.session_state.history.pop()
    st.rerun()

# CLEAR BUTTON
if col_clear.button("🗑️ Clear Roster", type="secondary", use_container_width=True):
    save_history()
    st.session_state.main_df = pd.DataFrame(columns=["Associate", "Role", "Shift", "Lunch Time", "StartDt", "EndDt", "Duration"])
    st.rerun()
...
with c1:
    with st.expander("➕ Manually Add Associate", expanded=False):
        ...
        m_shift = st.text_input("Shift (e.g. 4am-1pm)")
...
if not df.empty:
    ...
    target_role = b2.selectbox("Role:", ["Pickers", "Backroom", "Exceptions", "Exclude"])
    if b3.button("🚀 Apply"):
        save_history()
